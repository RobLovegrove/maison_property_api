...
@bp.route(
    "/<string:user_id>/transactions/<uuid:negotiation_id>/progress",
    methods=["GET"],
)
def get_transaction_progress(user_id, negotiation_id):
    ...
    try:
        ...
        progress = TransactionProgress.query.filter_by(
        ...
        db.session.rollback()
        current_app.logger.error(
            f"Error getting transaction progress: {str(e)}"
        )
...
def update_transaction_progress(user_id, negotiation_id):
    ...
    try:
        ...
        if str(user_id) != str(property_item.seller_id) and str(
            ...
            return (
                jsonify(
                    {"error": "Unauthorized to update transaction progress"}
                ),
                403,
            )
        ...
        db.session.rollback()
        current_app.logger.error(
            f"Error updating transaction progress: {str(e)}"
        )
        return jsonify({"error": "Failed to update transaction progress"}), 500


@bp.route(
    "/<string:user_id>/transactions/<uuid:negotiation_id>/progress/confirm",
    methods=["POST"],
)
def confirm_transaction_step(user_id, negotiation_id):
    ...
    try:
        ...
        if str(user_id) != str(property_item.seller_id) and str(
            ...
            return (
                jsonify({"error": "Unauthorized to confirm transaction step"}),
                403,
            )
        ...
        step_mapping = {
            "final_checks": (
                "buyer_final_checks_confirmed"
                if is_buyer
                else "seller_final_checks_confirmed"
            ),
            "exchange_contracts": (
                "buyer_exchange_contracts_confirmed"
                if is_buyer
                else "seller_exchange_contracts_confirmed"
            ),
            "completion": (
                "buyer_completion_confirmed"
                if is_buyer
                else "seller_completion_confirmed"
            ),
        }
        ...
        db.session.rollback()
        current_app.logger.error(
            f"Error confirming transaction step: {str(e)}"
        )
