...
def test_create_property(client, init_database, sample_property):
    ...
    assert data["address"]["street"] == (
        sample_property["address"]["street"]
    )
    assert data["specs"]["bedrooms"] == (
        sample_property["specs"]["bedrooms"]
    )


def test_get_property_detail(client, init_database, sample_property):
...
def test_update_property(client, init_database, sample_property):
...
def test_delete_property(client, init_database, sample_property):
    # Test DELETE /api/properties/<id> removes the property and related data
    # Create property first
    create_response = client.post("/api/properties", json=sample_property)
    property_id = create_response.json["id"]
    ...
    # Verify deletion of main property and related data
    get_response = client.get(f"/api/properties/{property_id}")
    assert get_response.status_code == 404


def test_create_property_validation(client, init_database):
...
def test_filter_properties(client, init_database):
    ...
    assert response.status_code == 200
    assert len(response.json) == 1
    assert response.json[0]["address"]["city"] == (
        "London"
    )


def test_advanced_filters(client, init_database):
    """Test advanced property filters"""
    # Create test property with specific features
    property_data = {
        "price": 450000,
        "status": "for_sale",
    ...
    filters = [
        ("min_square_footage=1400", 1),  # Properties >= 1400 sq ft
        ("bathrooms=2&has_garage=true", 1),  # 2+ bathrooms with garage
        ("epc_rating=A&has_garden=true", 1),  # A-rated with garden
        ("postcode_area=SW1", 1),  # Properties in SW1
        ("min_square_footage=2000", 0),  # No properties this large
    ]
...
def test_properties_default_ordering(client, init_database):
    """Test properties are returned newest first by default"""
    # Create properties with different timestamps
    properties = [
        {
            "price": 350000,
            "status": "for_sale",
            "description": "Older Property",
            "address": {
                "house_number": "123",
                "street": "Test Street",
                "city": "London",
            ...
            "features": {"has_garden": False, "parking_spaces": 0},
        },
        {
            "price": 750000,
            "status": "for_sale",
            "description": "Newer Property",
            "address": {
                "house_number": "456",
                "street": "Other Street",
                "city": "Manchester",
            ...
            "features": {"has_garden": True, "parking_spaces": 2},
        },
    ]
    ...
    assert response.json[0]["description"] == "Newer Property"
